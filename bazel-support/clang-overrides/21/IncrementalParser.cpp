...
//===----------------------------------------------------------------------===//
//
// CTC override of the LLVM 21.1.x archive member (see BUILD.bazel): backs
// incremental input buffers with virtual FileEntries so that directives which
// dereference the buffer's FileEntry (e.g. #pragma once) do not crash. Carries
// the upstream fix 9cc0df99de85 (llvm/llvm-project#182044). Remove this file
// when @llvm moves to a tree that contains the fix (LLVM >= 22).
//===----------------------------------------------------------------------===//
